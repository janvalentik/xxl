...
function _HCM_sbox($id=null, $chatmod=false, $chat_refresh=20){
  ...
  $result="";
  $id=intval($id);
  if(isset($chatmod) and $chatmod==1){$bezchatu=0;}else{$bezchatu=1;}
  $chat_refresh=intval($chat_refresh);
  if($chat_refresh>5){$rchatreload=intval($chat_refresh);}else{$rchatreload=20;}
  ...
  if($rcontinue){
    ...
    if($sboxdata['locked']!=1 and _publicAccess($sboxdata['public']) and _publicAccess($bezchatu)){
      ...
      if(!_loginindicator){$inputs[]=array($GLOBALS['_lang']['posts.guestname'], "<input type='text' name='guest' class='sbox-input' maxlength='22' />");}
      ...
      if(!_loginindicator){$inputs[1][2]=true; $inputs[]=$captcha;}
      
      //kontrolni prvky pro chat
      if($bezchatu==0){
        $rcontrols=
        _getPostformControls("hcm_sboxform_".$GLOBALS['__hcm_uid'], "text", true)." <span id='hcm_timer_".$GLOBALS['__hcm_uid']."' class='lpad'></span>
...
<noscript><a href='"._indexOutput_url."'>".$GLOBALS['_lang']['global.reload']."</a></noscript>";
    ...
    $result.=_formOutput(
    "hcm_sboxform_".$GLOBALS['__hcm_uid'],
    _indexroot."remote/post.php?_return=".urlencode(_indexOutput_url."#hcm_sbox_".$GLOBALS['__hcm_uid']),
    $inputs,
    null,
    null,
    $rcontrols
      ...
      if($sboxdata['locked']!=1){$result.=$GLOBALS['_lang']['posts.loginrequired'];}
      else{$result.="<img src='"._templateImage("icons/bubble.gif")."' alt='locked' class='icon' /> ".$GLOBALS['_lang']['posts.locked2'];}
    }
